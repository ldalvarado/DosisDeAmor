...
export class PostPetsCardComponent implements OnInit {
  ...
  user: User;
  isBrowser = false;
  fotoPerfil: any = window.localStorage['fotoPerfil'];
  comentarios= [];
  @Input()post: Post;
   ...
   */
  goToAccount(Post) {
    this.router.navigate(['/animal/profile-pet', Post.id_mascotas]);
  }
  /**
  ...
  mostrarMas(id_publicacion){
    
  }

  comentar(id_publicacion){
    if(this.message!==''){
      var headers = new HttpHeaders();
        const request = ({ headers: headers });
        const query = {
          comentario:this.message,
          id_mascota_coment: this.idMascota,
          publica_id: id_publicacion,
        }
        this.http.post(this.apiUrl + 'comentariosPublicaciones',query,request)
          .subscribe( data => {
      ...
      this.presentToast('Escriba algo porfavor');
    }
  }
  /**
   * Abre un alert.
...
export class PostPetsCardPopComponent implements OnInit {
